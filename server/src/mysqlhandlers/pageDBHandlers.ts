...
exports.getAllPagesRelatedToUserWithRoles = async function (userId) {
    ...
    pages.forEach(page => {
        role = ROLES.GUEST;
        if (page) {
            if (page.admin_id == userId) role = ROLES.ADMIN;
            else if (page.sub_admins_ids.split(',').includes(userId.toString())) role = ROLES.SUB_ADMIN;
            else if (page.mods_ids.split(',').includes(userId.toString())) role = ROLES.MOD;
            else if (page.users_ids.split(',').includes(userId.toString())) role = ROLES.USER;
